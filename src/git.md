...
```sh
# 可以查看本地分支对应的远程分支,列出所有本地分支，并显示它们的跟踪分支和最新的提交信息
git branch -vv 
# 查看远程版本库分支列表
git branch -r
# 查看所有分支列表，包括本地和远程
...
```
- 当cherry-pick出现冲突时，手动解决完冲突后，需要git add conflict_files，然后再执行continue命令。
...
- git merge 
  - 合并之后会增加一个合并节点。
- git rebase
  - 不会增加额外的合并节点。看不到其他分支的流动方式。隐藏了特性的来源分支。看到的特性是一条直线。
## 我的 alias 设置
```
a = add 
aa = add -A
...
## 致谢
* 感谢[git-tips](https://github.com/OneSizeFitsQuorum/git-tips/blob/main/README.md)
